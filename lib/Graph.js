...
function Graph (neo4j)
{
	...
	this.query = function (/* batch, profile, */ query, /* params, */ callback)
	{
		...
		new neo4j.Api.Request(args.batch, endpoint, 'POST', body, function (error, obj)
		{
			...
			// Loop through data
			function traverse(o, parent, pkey) {
				var m, key;
				if (typeof o !== 'object' && parent[pkey]) {
					parent[pkey] = o;
				}
				...
				if (!m) {
					for (key in o) {
						if (!o.hasOwnProperty(key)) {
							continue; // Make sure it's real data
						}
						m = match(o[key]);
						parent[pkey][key] = (m) ? m : traverse(o[key], parent[pkey], key);
					}
				}
				else {
					parent[pkey] = m;
			...
			function match(o) {
				...
				{
					if (typeof o.self === 'string' && typeof o.data === 'object')
					{
						// a relationship or node
						return typeof o.start === 'string' ? new neo4j.Relationship(o) : new neo4j.Node(o);
					}
					else if (typeof o.start === 'string' && typeof o.end === 'string' && typeof o.nodes === 'object')
					{
						//a path
						return new neo4j.Path(o);
					}
					else
					{
						// an object that isn't a path, relationship or node
						return false;
					}
				}
				else
				{
					// not an object or array, should be actual value
					return o;
				}
			}


			for (r = 0; r < obj.data.length; r++) {
				results.push({});
				for (c = 0; c < obj.columns.length; c++) {
					results[r][obj.columns[c]] = {};
					traverse(obj.data[r][c], results[r], obj.columns[c]);
				}
			}
